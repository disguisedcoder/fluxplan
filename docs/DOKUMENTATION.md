...
| `daily_focus` | `daily_focus` | Viele offene/heutige Aufgaben (Schwelle in Regel) | **Keine** Datenänderung — setzt `adaptive.dailyFocusListHighlight`: To‑Do‑Liste auf `/heute` zeigt dann auch **überfällige** Einträge und hebt überfällig + heute **rot** hervor; ohne Annahme bleiben Überfällige in dieser Liste ausgeblendet |
