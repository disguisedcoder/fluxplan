...
3. **Bei Adaptive:** Nach Navigations- und Aufgaben-Phasen ggf. kurz prüfen (ohne Proband zu führen):
   - Oben auf **`/heute`** ein Banner erscheinen kann (normal, wenn pending Vorschlag).
   - **`/anpassungen`** → Tab „Anpassungen“: pending Karten mit **farbigem Rand** (kein Bug).
...
| --- | --- | --- |
| **Eval-Runner** | Phase 2A Navigation, 2B viele Aufgaben mit Datum/Erinnerung | Banner; Vorschläge zu Chips oder Einklappen; `Anpassungen` füllt sich |
| **Taskplanner** | Filter, Erstellen, viele Tasks | Formular-Hinweise; Kalender + Liste |
| **Familienplanner** | Kalender, Termine, Dauer | Konflikt-Hinweis-Vorschlag; Kalender-Markierungen ohnehin sichtbar |
